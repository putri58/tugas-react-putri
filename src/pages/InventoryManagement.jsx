...
import { Package, Plus, Search, AlertTriangle, ArrowUpRight, Filter, CheckCircle } from "lucide-react";
...
export default function InventoryManagement() {
  ...
  const [inventory] = useState([
    ...
    { id: "INV-003", name: "Royal Canin Urinary S/O Kucing", category: "Pakan Khusus", stock: 25, unit: "Bag (2kg)", status: "Aman", expiry: "15 Jan 2027" },
    ...
    <div className="p-6 max-w-5xl mx-auto space-y-6 font-sans">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b pb-5">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">
            Pantau real-time sediaan obat, kontrol masa kedaluwarsa, dan kelola otomatisasi reorder barang otomatis.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-white p-4 border rounded-xl shadow-sm">
          <span className="text-xs text-gray-400 block font-bold uppercase">Total Item Terdaftar</span>
          <span className="text-2xl font-black text-gray-800 block mt-1">2,450 Item</span>
        </div>
        <div className="bg-amber-50/50 border border-amber-100 p-4 rounded-xl">
          ...
          <span className="text-2xl font-black text-amber-600 block mt-1">12 Item</span>
        </div>
        <div className="bg-rose-50/50 border border-rose-100 p-4 rounded-xl">
          <span className="text-xs text-rose-700 block font-bold uppercase">Mendekati Expired (&lt; 3 Bulan)</span>
          <span className="text-2xl font-black text-rose-600 block mt-1">4 Obat</span>
        </div>
      </div>
                ...
                <tr key={item.id} className="hover:bg-gray-50/50 transition-colors">
                  ...
                  <td className="p-4 text-center">
                    <span className={`text-[11px] font-bold px-2.5 py-0.5 rounded-full inline-block ${
                      item.status === "Aman" ? "bg-emerald-50 text-emerald-700" :
                      item.status === "Stok Menipis" ? "bg-amber-50 text-amber-700 border border-amber-100" :
                  ...
                  <td className="p-4 text-right">
                    <Button variant="outline" size="sm" className="text-xs h-8 text-indigo-600 border-indigo-100 hover:bg-indigo-50">
                      Restock
                    </Button>
                  </td>
                </tr>
